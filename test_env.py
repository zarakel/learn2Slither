...
def main():
    ...
    while not is_dead:
        ...
        # 1. On utilise le rendu de l'environnement pour afficher la vision en croix
        env.render()
        ...
        if mouvement in mapping_touches:
            ...
            # Elle renvoie 5 valeurs : observation, reward, terminated, truncated, info
            obs, reward, is_dead, truncated, info = env.step(action)
            
            print(f"Récompense reçue pour cette action : {reward}")
            input("Appuyez sur Entrée pour continuer...") # Pause pour lire la récompense
    ...
    if is_dead:
        os.system('cls' if os.name == 'nt' else 'clear')
        env.render()
        print("\n💥 GAME OVER ! L'agent a reçu la récompense fatale : -100.0")
